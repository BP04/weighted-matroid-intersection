...
vector<GroundSetElement> final_ground_set;
vector<Edge> forest_edges, final_forest_edges;
int cnt_forest = 0;
int special[55];
...
void build_answer(int pos = 0){
    ...
    if(pos == K){
        ...
        if(temp < answer){
            answer = temp;
            final_ground_set = ground_set;
            final_forest_edges = forest_edges;
        }
    ...
    for(int mask = 0; mask < (1 << pos); ++mask){
        ...
        for(int i = 0; i < pos; ++i){
            if(mask >> i & 1){
                cnt++;
                int u = special[i], v = special[pos];
                forest_edges.push_back({u, v, AdjMat[i][pos]});
            }
        }
        build_answer(pos + 1);
        while(cnt--){
...
void solve(){
    ...
    cout << "\nMinimum total weight = " << answer << "\n\n";

    cout << "Edges:\n";
    for(int i = 0; i < final_forest_edges.size(); ++i){
        cout << "u = " << final_forest_edges[i].from << ", v = " << final_forest_edges[i].to << ", w = " << final_forest_edges[i].weight << "\n";
    }

    for(int i = 0; i < final_ground_set.size(); ++i){
        if(final_ground_set[i].in_independent_set){
            cout << "u = " << final_ground_set[i].from << ", v = " << final_ground_set[i].to << ", w = " << final_ground_set[i].weight << "\n";
        }
    }
}
